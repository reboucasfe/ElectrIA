...
const ProposalForm = () => {
  ...
  const [selectedServiceToAdd, setSelectedServiceToAdd] = useState<string>(''); // ID do serviço a ser adicionado
  ...
  const handleAddService = () => {
    ...
    if (serviceToAdd) {
      ...
      setSelectedServiceToAdd(''); // Clear selection
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="flex justify-end gap-4">
        ...
        <Button type="button" onClick={() => showSuccess("Funcionalidade de IA em desenvolvimento!")} disabled={loading} className="bg-blue-600 hover:bg-blue-700">
          {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null} Gerar Proposta com IA
        </Button>
      </div>
    </form>
  );
